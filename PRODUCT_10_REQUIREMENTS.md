# 苍穹战机 · 产品向 10 条需求拆解（用户故事 + 验收标准 + 红线确认）

> 作者：许清楚（product-manager / pm-req）｜日期：2026-08-30
> 范围：已拍板 A+B 全做的 10 条产品向优化。本文只定义「做什么 / 怎么算做对」，
> 不写实现细节（落点标注供架构师参考，非实现方案）。
...
- GIVEN 关卡配置含 `waveVariants`（每关 2~3 套同难度变体波次表，纯数据，追加在 `LEVELS[i]`）
  WHEN 玩家进入该关 WHEN 局开始，THEN 系统按局随机锁定 1 套变体，本局所有波次均用该套表（同局内不换表）。
- GIVEN 连续两次重开同一关，WHEN 对比两次的波次敌人组合，THEN 至少 40% 概率（抽到不同变体时）两次组合不同；且两套变体均为同难度系数（difficulty 不变、敌人血量/速度/弹速系数不变）。
- GIVEN 第 N 波使用变体表，WHEN 波次生成，THEN 敌人数与敌人组合（count/comp）与所选变体的第 N 项一致（沿用 WaveSystem 既有权重抽取，仅替换数据来源）。
- GIVEN 某关没有 `waveVariants`（旧配置/兜底），WHEN 进关，THEN 回退到既有 `wavePlan`，行为与现状逐帧等价（零回归）。
- GIVEN 无尽 / 爬塔 / BossRush / 活动模式，WHEN 进局，THEN 不受影响（无尽本就程序化随机；BossRush 无波次）。
...
### 验收标准（Given / When / Then）
- GIVEN 存档满足 skyOverlord 三项条件，WHEN 结算页加载，THEN 显示「苍穹霸主 / Sky Overlord」称号。
- GIVEN 存档仅满足 combo_30，WHEN 结算页加载，THEN 显示「连击狂人 / Combo Maniac」（不因更高阶未达成而空白）。
- GIVEN 存档为全新账号（无任何达成），WHEN 结算页加载，THEN 不显示称号或显示「苍穹新兵」（取决于是否通关过一关），不报错。
- GIVEN 结算页展示称号，WHEN 分享卡生成，THEN 卡片上包含同一称号文本。
- GIVEN 语言为 en，WHEN 结算页/分享卡展示称号，THEN 使用英文称号名。
...
- 性能档无粒子新增，N/A。
- i18n：新增 `title_rookie`…`title_skyOverlord` 等 8 组中英文案（见第 12 节词表）。
...
### 新增能力
- **昵称**：本地自定义昵称，默认「飞行员 / Pilot」，可改（≤8 字符），存 `SaveManager.nickname`（append-only）。编辑入口：设置面板新增昵称输入（实现方可用 DOM 覆盖式输入或字符选择，PM 只定义行为）。
- **称号**：复用第 2 条派生结果。
...
### 验收标准（Given / When / Then）
- GIVEN 存档已保存昵称「阿飞」，WHEN buildShareCard 生成画布，THEN 画布文本包含「阿飞」；未设置时显示默认「飞行员」。
- GIVEN 玩家已解锁某称号，WHEN buildShareCard，THEN 画布包含该称号文本（与结算页一致）。
- GIVEN 第 3 关（敌方要塞）通关，WHEN buildShareCard，THEN 画布背景渐变使用第 3 关 theme 的 skyTop/skyBottom 色值（断言 canvas 像素或数据源）。
- GIVEN 本局分数 > 同关同模式历史最高分，WHEN buildShareCard，THEN 显示「比上次 +{pct}%」（pct=向上取整百分比）或「★新纪录」。
- GIVEN 无同关同模式历史记录，WHEN buildShareCard，THEN 显示「首秀」，不报错、不显示异常百分比。
...
- 昵称输入 ≤8 字符；不做敏感词过滤（本地单机）。

### 红线确认
- SaveManager 仅 append-only 新增 `nickname`（默认 ''→展示层回退「飞行员」）；不碰旧字段。
...
### 验收标准（Given / When / Then）
- GIVEN 玩家 normal 第 2 关连续失败 3 次，WHEN 再次进入第 2 关，THEN 弹出救济提示面板（含「降低难度 / 临时增益 / 拒绝」三个选项）。
- GIVEN 玩家选择「临时增益→+1 命」，WHEN 本局开始，THEN 本局命数 +1 且攻击加成（若选 +10%）本局生效；下一局重新开始时无残留。
- GIVEN 救济局中玩家复活，WHEN 复活完成，THEN 追加临时火力 +1 持续 2 秒，2 秒后恢复原火力（局内 powerLevel 数值不变）。
- GIVEN 救济局胜利，WHEN 结算完成，THEN topScores / levelMedals / achievements / league 均无本局记录，`failStreak[levelId]` 归 0。
- GIVEN 玩家在救济提示选「降低难度」，WHEN 本局进行，THEN 敌人/Boss 系数按休闲档；`selectedDifficulty` 存档值不变。
...
### 验收标准（Given / When / Then）
- GIVEN 玩家首次击杀 turret，WHEN 击杀结算完成，THEN `codex.enemies.turret = true` 且图鉴页该条目点亮。
- GIVEN 玩家首次击败 boss_annihilator，WHEN Boss 击败，THEN `codex.bosses.boss_annihilator = true`。
- GIVEN 玩家首次使用 laser 武器，WHEN 武器切换，THEN `codex.weapons.laser = true`。
- GIVEN 图鉴页加载，WHEN 渲染条目，THEN 已解锁条目显示名称，未解锁显示「???」，计数（已解锁 12/18）正确。
- GIVEN 玩家购买图鉴装饰，WHEN 金币足够，THEN 扣金币、`codexDecor` 追加记录、图鉴页样式变化；金币不足时不扣不记。
...
### 验收标准（Given / When / Then）
- GIVEN 当前连击 ≥10 且玩家点按「蓄力→强化射击」，WHEN 触发，THEN 当前连击归 0（HUD 显示 0），强化射击 buff 生效 3 秒。
- GIVEN 当前连击 ≥15 且触发「清屏」，WHEN 触发，THEN 全场敌弹清除、全场敌机受中等伤害、当前连击归 0、炸弹数量不变。
- GIVEN 当前连击 ≥20 且触发「回能」，WHEN 触发，THEN 能量槽充满、当前连击归 0。
- GIVEN 本局此前峰值已达 30（combo_30 已解锁），WHEN 玩家消耗连击至 0，THEN `session.comboPeak` 保持 30，combo_30 仍为已解锁、不重复弹提示。
- GIVEN 当前连击 <10，WHEN 玩家点按蓄力，THEN 不触发任何效果（按钮置灰/无响应），连击不被消耗。
...
- **间隔**：每 5 层触发一次全局变异（`towerFloor % 5 === 0`，即 Boss 波通关后进入第 5/10/15… 层时）。⚠️ 注：按 TOWER.BOSS_EVERY=10 波/层，5 层 ≈ 50 波，节奏偏长；若实测偏慢，可调配置为「每 5 波」——**以配置常量表达，不改框架**（PM 建议：先按「每 5 层」，真测后由主理人拍板是否改 5 波）。
...
### 验收标准（Given / When / Then）
- GIVEN 爬塔层数到达 5，WHEN 进入第 5 层，THEN 弹出变异横幅并随机应用 1 个正面或负面变异（正负比例建议 55/45，可配置）。
- GIVEN 正面变异「磁力风暴」生效，WHEN 玩家附近有金币，THEN 金币被磁力吸附（磁力效果持续至下次变异）。
- GIVEN 负面变异「弹速风暴」生效，WHEN 敌人开火，THEN 敌弹速度 ×1.2；生效前出现 ≥1 秒警示文字。
- GIVEN 爬塔层数到达 10，WHEN 进入第 10 层，THEN 上一变异结束、新变异生效（可相同可不同）。
- GIVEN 普通关 / BossRush / 活动模式，WHEN 进局，THEN 无任何变异逻辑生效（零回归）。
...
### 验收标准（Given / When / Then）
- GIVEN selectedDifficulty = casual 或 standard，WHEN 任意波次生成，THEN 不出现任何带免疫标记的敌人。
- GIVEN selectedDifficulty = hard/hell 且 wavePlan 某条目带 `immune:'fire'`，WHEN 该敌人生成，THEN 敌人体表显示「免疫火」标记，且被火元素/火 DoT/火反应命中时伤害为 0。
- GIVEN 同一火免疫敌人被冰元素命中，WHEN 命中结算，THEN 冰伤害正常、减速状态正常生效。
- GIVEN 场上 active 敌机同时存在 fire/ice/thunder 三种 `_elem`，WHEN 条件满足，THEN 触发全屏元素风暴（清敌弹 + 穿透免疫伤害 + 得分），且 15 秒内不重复触发。
- GIVEN 风暴冷却中，WHEN 再次满足三元素同挂，THEN 不触发，待冷却结束后可再次触发。
...
### 验收标准（Given / When / Then）
- GIVEN selectedDifficulty = standard/hard/hell 且某波刷出精英，WHEN 敌人生成，THEN 该敌机体型略大、带发光描边、血量 = 基础 ×5 × 关卡难度 × 难度档。
- GIVEN 精英被击杀，WHEN 击杀结算，THEN 走正常 registerKill（击杀数 +1、连击 +1、得分 ×3），不触发任何 Boss 事件，`bossesDefeated` 不变。
- GIVEN 精英被击杀，WHEN 掉落结算，THEN 必掉 1 个 BOSS_DROP_TABLE 高价值道具（energy/heal/wingman/bomb/weapon 之一）。
- GIVEN selectedDifficulty = casual，WHEN 任意波次生成，THEN 不出现精英。
- GIVEN 第 1 波（每关开局波），WHEN 生成，THEN 不刷精英（避免开局突袭）。
...
### 验收标准（Given / When / Then）
- GIVEN Boss hp 降至 maxHp × 15% 以下，WHEN 本次命中结算，THEN Boss 进入狂暴态：入场演出 ≥1.2s、红屏 + 标题提示、DPS 检查开始（横幅显示「8 秒内造成 X 伤害」）。
- GIVEN 狂暴态持续，WHEN 观察弹幕，THEN 任意时刻存在可穿行的安全缝隙（缺口宽 ≥ 玩家机身 3 倍），且每两组全屏弹幕间有 ≥0.5s 间歇。
- GIVEN 玩家在 8s 窗口内造成 ≥ maxHp×10% 伤害，WHEN 窗口结束，THEN Boss 进入「破绽」2s（受击 ×2、硬直）。
- GIVEN 玩家未在窗口内达成 DPS，WHEN 窗口结束，THEN Boss 回复至 maxHp × 20% 并释放一次全屏弹幕；之后被再次压至 15% 可重新进入狂暴（不卡死）。
- GIVEN 狂暴态中 Boss 被击杀，WHEN 死亡结算，THEN 走正常 BOSS_DEFEATED 流程：掉落/成就/爬塔层数各只记一次，不重复。
...
## 十一、新增存档字段汇总（全部 append-only）

| 字段 | 类型/默认 | 所属条目 | 说明 |
|---|---|---|---|
| `nickname` | `''`（展示回退「飞行员」） | 3 | 本地自定义昵称，≤8 字符 |
...
## 十二、新增 i18n 词表（zh/en，开发落地时补入 Locale.js，全部 append-only）
...
## 十三、风险与开放问题（已做 PM 决策，标注可推翻项）
> ✅ 主理人拍板记录（2026-08-30）：以下 1/2/3 三项均已确认，本文按拍板结论为准。
1. ✅ 拍板：第 4 条救济局「不计 bestScore / 每日任务 / 新手计划」采纳（与 A9 语义统一，防救济局刷榜刷任务）。最终救济局口径 = 不计 `topScores / levelMedals / levelStars / league / bestScore / 每日任务 / 新手计划`，仅 `failStreak / reliefRuns` 计数（金币照常入账）。
2. ✅ 拍板：第 7 条变异间隔按需求原文「每 5 层」执行，用配置常量表达（`MUTATION_EVERY_LAYERS: 5`），真测后如需调整只改常量、不改框架。
3. ✅ 拍板：第 3 条昵称本批先做「默认昵称 + 随机后缀」（如 飞行员·42），分享卡显示昵称；昵称编辑文本框后置 P2（不阻塞本批交付）。
4. 第 4 条救济提示「降低难度」与既有四档难度选单并存：救济降难度是 session 覆盖，返回菜单后恢复玩家原选择（体验一致）。
5. 第 8 条三元素同挂风暴的「元素状态」读 `Enemy._elem`，需注意元素状态过期/敌机死亡的即时清理，避免死敌残留造成误触发（实现方加活性守卫，QA 专项断言）——主理人已确认在实现规格中写明。
