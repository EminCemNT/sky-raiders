...
新增 2 个成就：
- `combo_element_5` —— 单局触发 5 次元素协同
- `combo_element_50` —— 累计触发 50 次（跨局持久化，存档字段 `elementCombos`）
...
| **P2-3** | combo 半常驻 | `TRIGGER: 3` 太低，实测增益占空比高达 52%~76%，"协同爆发"变成常态背景 | 交付总监拍板 `TRIGGER: 3 → 5`，实测占空比降至 **18%~28%**（QA 12s 真实波次测得 18%，实现者合成场景测得 28%） |
...
- P2-3 —— 真实波次 12s 增益占空比从 52% 降至 **18%**；5 次交替仍能触发，触发后 count 正确清零
...
| combo 视觉与数值精调 | 占空比已从 76% 降至 18%~28%，但 1.35 倍率与 3 秒时长仍可再打磨 |
| **成就阈值需 PM 复评**（QA 提出） | `TRIGGER: 3→5` 的连带影响：每次 buff 激活的门槛从 3 次交替升到 5 次，`combo_element_5` / `combo_element_50` 在短局中明显更难达成。这是 P2-3 拍板的预期代价、**非回归 bug**，但阈值是否需要下调请产品侧复评 |
